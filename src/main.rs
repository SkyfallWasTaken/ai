...
async fn index(data: web::Data<AppState>) -> Result<impl Responder, Box<dyn std::error::Error>> {
    let sum: i32 = if let Some(pool) = &data.db_pool {
        match pool.get().await {
            Ok(conn) => {
                match conn.query_one("SELECT COALESCE(SUM((response->'usage'->>'total_tokens')::real), 0)::integer as total FROM api_request_logs WHERE response->'usage'->>'total_tokens' IS NOT NULL", &[]).await {
                    Ok(row) => row.get::<_, i32>("total"),
                    Err(e) => {
                        log::error!("Failed to query token sum: {}", e);
                        -1
                    }
                }
            }
